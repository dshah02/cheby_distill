#!/bin/sh
#SBATCH --job-name=train_pairs # create a short name for your job
#SBATCH --output=slurm/slurm_output/tp/%x_%j.out
#SBATCH --error=slurm/slurm_output/tp/%x_%j.err
#SBATCH --nodes=1                # node count
#SBATCH --ntasks-per-node=1      # crucial - only 1 task per dist per node!
#SBATCH --cpus-per-task=1      # cpu-cores per task (>1 if multi-threaded tasks)
#SBATCH --mem-per-cpu=8G                 # total memory per CPU core (4 GB per cpu-core is default)
#SBATCH --gres=gpu:1
